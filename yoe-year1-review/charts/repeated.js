...
const intervalNormalDistribution = (ids, field) => {
    const grid = [];
    const xAxis = [];
    const yAxis = [];
    const dataset = [];
    ...
    ids.forEach((id) => {
        ...
        for (let interval = 0; interval < sd.data.length; interval++) {
            const mean = calcMean(sd.data[interval].map((i) => i[field]));
            const standardDeviation = calcStandardDeviation(
                sd.data[interval].map((i) => i[field]), mean
            );
...
export const repeated = {
    ...
    interval_3_2000_300: {
        ...
        intervalNormalDistribution: () => intervalNormalDistribution([ 40793098 ], 'p'),
    },
};
