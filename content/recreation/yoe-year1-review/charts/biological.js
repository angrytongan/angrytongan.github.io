...
const units = (metric, value) => {
    switch (metric) {
        case 'haemoglobin': return `${value.toFixed(0)}Hb`;
        case 'rhr': return `${value.toFixed(0)}bpm`;
        case 'weight': return `${value.toFixed(1)}kg`;
        case 'bp': return `${value.toFixed(0)}mmHg`;
    }
};
const bounds = {
...
const summary = () => {
    ...
    xAxis.push({
        ...
        gridIndex: 1,
        axisLabel: {
            formatter: (value) => echarts.time.format(value, '{yyyy}-{MM}-{dd}', false),
        },
        min: '2019-09-16',
        max: '2020-09-13',
    });
...
const distribution = () => {
    ...
    const offset = 4;
    grid.push(
        { left: offset + 0 * width + '%', right: offset + 2 * width + '%', bottom: 45 },
        { left: offset + 1 * width + '%', right: offset + 1 * width + '%', bottom: 45 },
        { left: offset + 2 * width + '%', right: offset + 0 * width + '%', bottom: 45 },
    );
